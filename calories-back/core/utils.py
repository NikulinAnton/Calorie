...
from core.models import DietFood, Food, CUser
...
def update_dietfood(diet_food_instance: DietFood, validated_data: dict, owner: CUser) -> DietFood:
    food_data = validated_data.pop('food')
...
def _update_or_create_food(food_data: collections.OrderedDict, owner: CUser) -> Food:
    food, _ = Food.objects.update_or_create(name=food_data.get('name'), owner=owner, defaults=dict(calories=food_data.get('calories'), proteins=food_data.get('proteins'), carbs=food_data.get('carbs'), fats=food_data.get('fats')))
    return food